...
bot.on(message("text"), async (ctx) => {
  logger.info(`Received message from ${ctx.message.from.id}: ${ctx.message.text}`);
  const replyMessage = await processExpense(ctx.message.text, ctx.message.from.id.toString());
